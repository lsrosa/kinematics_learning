...
%
In the fist step, we train $\nn$ and a policy $\policy:\realSet{\nDims\times\nJoints}\rightarrow\realSet{\nJoints}$, which given the Cartesian position of the robot joints, learns the torques for each joint for moving the end-effector towards the target position.
%
In this first step, we use random sampling for learning $\nn$, and the ground truth values from simulation for learning $\policy$.
%
