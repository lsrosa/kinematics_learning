...
%
Next, $nIterations$ of training is done with the data already within the memory, which are done by randomly sampling $batchSize$ samples from the memory (line $8$), computing the average loss (lines $9$ to $14$), which is then back-propagated through the network (line $15$) with an $\learningRate$ learning rate.
...
\begin{algorithm}
	...
	\For{$e \in \{1,\hdots, epochs\}$}{
		...
		$meanLoss\leftarrow meanLoss/batchSize$\;
