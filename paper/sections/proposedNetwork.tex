...
\begin{figure}
	...
	\caption{Proposed architecture. For each joint $i\in\{1,\hdots,j\}$, a network $\Phi_i$ learns the \gls{dh} transformation from joint $i-1$ to $i$ $\matrix{R}_{i-1}^{i}$.
	%
...
For training $\nnMono$, we follow the approaches in the literature \rev{citation needed} and use the average Euclidean distance between the joints and their respective prediction, as described in Equation \ref{eq: loss mono}.
%
...
\begin{equation}\label{eq: loss mono}
	\lossMono = \frac{1}{\nJoints}\sum_{k=1}^{\nJoints}||\predMono_k-\vector{x}_k||
\end{equation}
...
\rev{add the division by the sum of times lq is computed}

\rev{probably should prepare an ablation figure for these losses}
...
%
The memory size is limited, and upon additions further than its limit, it drops the oldest data.
%
...
\begin{algorithm}
	...
	\For{$e \in \{1,\hdots, epochs\}$}{
		...
		\For{$t\in\{1,\hdots,nIterations\}$}{
			$(qs, xs)\leftarrow$ randomly select $batchSize$ samples from $memory$\; 
			$meanLoss\leftarrow0$\;
			\For{$(q, x) \in (qs, xs)$}{
