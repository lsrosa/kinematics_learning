...
class PusherEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def calc_reward(self, d1, d2, v1, info):

        # print(f"d1 {d1:.3f} | d2 {d2:.3f} | v1 {v1:.3f}")

        dn1 = np.clip(1-d1/1.0, 0, 1)          # [0,0.4] -> [1,0]
        dn2 = np.clip(1-d2/0.4, 0, 1)          # [0,0.4] -> [1,0]
        vn1 = np.clip(1-v1/12, 0, 1)           # [0,12]  -> [1,0]

        # vn1d = np.clip(math.pow(vn1,dn1+dn2), 0, 1)   
        vn1d = np.clip(math.pow(vn1,2*dn2), 0, 1)

        g = 1.0 if (d2<0.06  and v1<1.0) else 0.0

        rv = dn1 * dn2 * vn1d

        info['reward_goal'] = g
        info['reward_vel'] = vn1d
        info['reward_dist1'] = dn1
        info['reward_dist2'] = dn2
        # info['reward_ctrl'] = np.linalg.norm(action)

        return (rv + g) / 2 # normalize to max 100 (max_episode_steps=100)

    def step(self, a):
        vec_1 = self.get_body_com("object") - self.get_body_com("tips_arm")
        vec_2 = self.get_body_com("object") - self.get_body_com("goal")
        vel = self.unwrapped.data.qvel.flat[:7]

        # velocity limit
        for i in range(vel.shape[0]):
            if vel[i]>10 and a[i]>0:
                a[i] = 0
            elif vel[i]<-10 and a[i]<0:
                a[i] = 0
        ...
        if False:
            ...
            reward = self.calc_reward(d1,d2,v1,info)
